...
\begin{enumerate}
\item\brm{2} Un losange peut avoir des côtés de longueurs différentes. $\cdotsx$
\item\brm{2} Le carré est un losange. $\cdotsx$
\item\brm{2} Le rectangle a forcément les diagonales de même longueurs. $\cdotsx$
\item\brm{2} Un rectangle est forcément un parallélogramme. $\cdotsx$
\item\brm{2} N'importe quel quadrilatère qui a un angle droit est un rectangle. $\cdotsx$
\end{enumerate}
\end{exercice}
...
\begin{enumerate}
\item\brm{2}\brm[0.5]{2} Calculer la mesure des angles $\widehat{IAB}$ et $\widehat{ICD}$.\newline
\anserline[2]
\item\brm{2} Déduire la mesure de $x$.\newline
\anserline[4]
\end{enumerate}
\end{minipage}%
\begin{minipage}{.5\linewidth}
